...
const PrecioEdit = (options: IProps) => {
  const queryClient = useQueryClient();
  const { show, handleClose, precio } = options;
  const [values, setValues] = useState<IPrecio>({
    liga: precio.liga,
  ...
  const { isLoading: isLoadingForm, mutate: mutateForm } = useMutation({
    mutationFn: (values: any) => createOrUpdatePrecio(values.body, values.id),
    onSuccess: (data) => queryClient.invalidateQueries(["precios"]),
  });
    ...
    <>
      <Modal className="" show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Editar {precio.usuario}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
        ...
        <Modal.Footer>
          ...
          <Button disabled={isLoadingForm} variant="primary" onClick={onSubmit}>
            {isLoadingForm ? "Actualizando precio..." : "OK"}
          </Button>
        </Modal.Footer>
      </Modal>
    </>
