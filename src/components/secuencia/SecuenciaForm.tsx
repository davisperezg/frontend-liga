...
interface IProps {
  show: boolean;
  handleClose: () => void;
  initialValue: ISecuencia;
}

const SecuenciaForm = (options: IProps) => {
  const queryClient = useQueryClient();
  const { show, handleClose, initialValue } = options;
  const [values, setValues] = useState<ISecuencia>(initialValue);
  ...
  const { isLoading: isLoadingForm, mutate: mutateForm } = useMutation({
    mutationFn: (values: any) => createOrUpdateSecuencia(values),
    onSuccess: (data) => {
      const { seq } = data;
      queryClient.setQueryData(["secuencias"], (prevModulos: any) => {
        return prevModulos.response.concat(seq);
      });
      queryClient.invalidateQueries(["secuencias"]);
    },
  });
  ...
  return (
    <Modal className="" show={show} onHide={handleClose}>
      <Modal.Header closeButton>
        <Modal.Title>Crear secuencia</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
